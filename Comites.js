...
}, 10000); // Cambiado a 10 segundos  img uadec
...
const eventProgram = document.querySelector('.event-program');

function showNextDay() {
    days[currentDay].classList.remove('active');
    currentDay = (currentDay + 1) % days.length;
    days[currentDay].classList.add('active');
    eventProgram.style.height = `${days[currentDay].offsetHeight}px`;
}

setInterval(showNextDay, 7000);
window.onload = () => {
    eventProgram.style.height = `${days[0].offsetHeight}px`;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    body.addEventListener('click', function (event) {
        if (!sidebar.contains(event.target) && !menuCheckbox.contains(event.target) && window.innerWidth >= 768 && window.innerWidth <= 1366) {
            hideSidebar();
        }
    });
